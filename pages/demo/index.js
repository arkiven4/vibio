import Head from "next/head";
import Image from "next/legacy/image";
import styles from "../../styles/Home.module.css";
import stylesCustom from "../../styles/custom.module.css";
import { useRouter } from "next/router";
import { motion } from "framer-motion";
import React, { useState, useEffect, useRef } from "react";

import { getJSONCategory } from "../../utils/getLocalJSON";
import { getLocale } from "../../utils/getLocaleText";
import { FooterLogo } from "../../components/general";

import { Walktour } from "walktour"

//TODO : Atur Category Card agar kalau jumlah itemnya 6, bagus keliatanya
//TODO : Fit Screen No Scroll untuk pad dan Laptop

export default function HomePlay(props) {
  const router = useRouter();
  const [pickGame, setPickGame] = useState(false);
  const [gameType, setGameType] = useState("");
  const [tourIndex, setTourIndex] = useState(0);

  const kategoriObj = props.kategori_data;
  const localeGeneral = props.localeData.general;

  const kategoriArray = [];
  var tempArray = [];
  let isPotrait = false;

  if (typeof window !== "undefined") {
    if (window.innerHeight > window.innerWidth) {
      isPotrait = true;
    }
  }

  let widthPercent = (100 / Object.keys(kategoriObj).length);
  let classCard_pL = isPotrait ? "col-4 col-sm-4 col-md-3 mb-4" : "col-4 col-sm-4 col-md-2 mb-4";

  Object.keys(kategoriObj).map((key, id) => {
    tempArray.push(
      <div key={id} onClick={() => router.push({ pathname: "/demo/" + gameType + "/" + key })} className={classCard_pL} style={!isPotrait ? {width: widthPercent + "%"} : {width: "40%"}}>
        <div className="card d-flex justify-content-center" style={!isPotrait ? { height: "45vh" } : null}>
          <Image src={`/assets/main_menu/${key}.png`} width={"600vw"} height={"600vw"} alt="PlayButton"></Image>
          <h2 style={{ textAlign: "center", fontSize: "3vw" }}>{kategoriObj[key].show_name} &rarr;</h2>
        </div>
      </div>
    );

    if (isPotrait) {
      if ((id + 1) % 2 == 0 && id != 0) {
        kategoriArray.push(tempArray);
        tempArray = [];
      }
      if (id + 1 == Object.keys(kategoriObj).length) {
        kategoriArray.push(tempArray);
        tempArray = [];
      }
    } else {
      if (id + 1 == Object.keys(kategoriObj).length) {
        kategoriArray.push(tempArray);
        tempArray = [];
      }
    }
  });

  function setGame(type) {
    setGameType(type);
    setPickGame(true);
  }

  return (
    <div className={(styles.container, stylesCustom.backgound_image)} style={{ backgroundImage: "linear-gradient(rgba(36, 36, 36, 0.40), rgba(36, 36, 36, 0.40)), url('/bg.jpg')" }}>
      <Head>
        <title>Vibio</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
      </Head>
      <Walktour
        isOpen={!pickGame ? true : false}
        steps={[
          {
            selector: "#playContainer",
            title: "Jenis Permainan",
            description: "Akan Ada 3 Jenis Permainan",
            disableMaskInteraction: true,
            disableClose: true,
            disableSmoothScroll: true,
            closeLabel: "",
            orientationPreferences: ["center"],
          },
          {
            selector: "#play_cardDengarkan",
            title: "Dengarkan Gambar",
            description: "Berfungsi untuk melatih ....",
            disableMaskInteraction: false,
            disableClose: true,
            disableSmoothScroll: true,
            closeLabel: "",
            orientationPreferences: ["south"],
          },
          {
            selector: "#play_cardMengeja",
            title: "Mengeja Gambar",
            description: "Berfungsi untuk melatih ...",
            disableMaskInteraction: false,
            disableClose: true,
            disableSmoothScroll: true,
            closeLabel: "",
            orientationPreferences: ["south"],
          },
          {
            selector: "#play_cardTebak",
            title: "Tebak Gambar",
            description: "Berfungsi untuk melatih ...",
            disableMaskInteraction: false,
            disableSmoothScroll: true,
            disableClose: false,
            closeLabel: "Tutup",
          },
          {
            selector: "#playContainer",
            title: "Pilih Jenis Permainan",
            description: "Silahkan pilih salah satu untuk melanjutkan demo",
            disableMaskInteraction: false,
            disableClose: true,
            disableNext: true,
            disablePrev: true,
            disableSmoothScroll: true,
            closeLabel: "",
            orientationPreferences: ["north"],
          },
        ]}
      />
      <Walktour
        isOpen={pickGame ? true : false}
        steps={[
          {
            selector: "#play_catgoryBenda",
            title: "Pilih Kategori Benda",
            description: "Tekan Pada Kartu Benda",
            disableMaskInteraction: false,
            disableClose: true,
            movingTarget: true,
            orientationPreferences: ["north"],
          },
        ]}
      />
      <main className="container">
        <motion.div
          className={styles.main}
          animate={pickGame ? "closed" : "open"}
          variants={{
            open: { opacity: 1, x: 0 },
            closed: { opacity: 0, x: "-100%" },
          }}
        >
          <div className="mt-5">
            <h2 className={stylesCustom.menu_title_font}>{localeGeneral.play_title1}</h2>
            <h4>{localeGeneral.play_subtitle2}</h4>
            <br></br>
          </div>

          <div className="row mb-3 d-flex justify-content-center" id="playContainer">
            <div onClick={() => setGame("dengarkan-gambar")} className="col-6 col-sm-6 col-md-4 mt-2" id="play_cardDengarkan">
              <div className="card">
                <Image className="card-img-top" style={{ borderRadius: "30px" }} src={`/assets/vector/dengarkan-image.jpg`} width={300} height={300} alt="dengarGambar"></Image>
                <h2 style={{ textAlign: "center", wordWrap: "break-word" }}>{localeGeneral.play_choose_title0} &rarr;</h2>
                <p style={{ textAlign: "center", wordWrap: "break-word" }}>{localeGeneral.play_choose_subtitle0}</p>
              </div>
            </div>
            <div onClick={() => setGame("mengeja-gambar")} className="col-6 col-sm-6 col-md-4 mt-2" id="play_cardMengeja">
              <div className="card">
                <Image style={{ borderRadius: "30px" }} src={`/assets/vector/mengeja-image.jpg`} width={300} height={300} alt="mengejaGambar"></Image>
                <h2 style={{ textAlign: "center", wordWrap: "break-word" }}>{localeGeneral.play_choose_title1} &rarr;</h2>
                <p style={{ textAlign: "center", wordWrap: "break-word" }}>{localeGeneral.play_choose_subtitle1}</p>
              </div>
            </div>
            <div onClick={() => setGame("tebak-gambar")} className="col-6 col-sm-6 col-md-4 mt-2" id="play_cardTebak">
              <div className="card">
                <Image style={{ borderRadius: "30px" }} src={`/assets/vector/tebak-image.jpg`} width={300} height={300} alt="tebakGambar"></Image>
                <h2 style={{ textAlign: "center", wordWrap: "break-word" }}>{localeGeneral.play_choose_title2} &rarr;</h2>
                <p style={{ textAlign: "center", wordWrap: "break-word" }}>{localeGeneral.play_choose_subtitle2}</p>
              </div>
            </div>
          </div>
          <FooterLogo></FooterLogo>
        </motion.div>
        <motion.div
          className={styles.main}
          animate={pickGame ? "open" : "closed"}
          variants={{
            open: { opacity: 1, x: 0 },
            closed: { opacity: 0, x: "-100%" },
          }}
          style={{ position: "absolute", top: "0", left: "0", opacity: "0" }}
        >
          <h2 className={stylesCustom.menu_title_font}>{localeGeneral.play_title2}</h2>
          <h4>{localeGeneral.play_subtitle2}</h4>
          <br></br>

          <div className="mx-4" id="play_catgoryBenda">
            {kategoriArray.map((value, id) => {
              return (
                <div key={id} className="row d-flex justify-content-center">
                  {kategoriArray[id]}
                </div>
              );
            })}
          </div>
          <FooterLogo></FooterLogo>
        </motion.div>
      </main>
      <div className={stylesCustom.home_button} onClick={() => router.push("/home")}>
        <div className={stylesCustom.button_card}>
          <h5 style={{ marginBottom: "0px", color: "green" }}>Home</h5>
        </div>
      </div>
      <audio controls loop autoPlay src={"/assets/music/bg-music1.wav"} style={{ display: "none" }}></audio>
    </div>
  );
}

// export async function getStaticProps(context) {

//   return {
//     props: {
//       localeData: {
//         general: localeDataGeneral,
//       },
//     },
//   };
// }

export const getStaticProps = async (context) => {
  var kategori_data = getJSONCategory();
  var localeDataGeneral = getLocale("id", "general");
  return {
    props: {
      kategori_data: kategori_data,
      localeData: {
        general: localeDataGeneral,
      },
    },
  };
};
